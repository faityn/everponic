...
import { galleryDetailAtom } from "../atom";
import { useRouter } from "next/router";
import { getGalleryDetail } from "@src/hooks/useData";
...
const GalleryDetail = () => {
  ...
  const getData = async () => {
    const response = await getGalleryDetail(Number(id));
        ...
        <div className=" max-w-[1800px] mx-auto w-full pt-24 pb-24">
          <div className="mt-20 grid grid-cols-10 ">
            ...
            <div className="col-span-6 max-md:col-span-12 w-full">
              <div>
                {itemsDetail[0]?.galleryFile?.length > 0 && (
                  <div className="mb-3 w-full">
                    <img
                      src={`${process.env.NEXT_PUBLIC_API_URL}/uploads/${itemsDetail[0]?.galleryFile[0]?.file_name}`}
